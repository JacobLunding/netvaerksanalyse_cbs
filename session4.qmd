...
[Download Danish Elite Network 2017](data/den17-no-nordic-letters.csv){.btn .btn-primary} 

og læg den i `/data` mappen
...
I det følgende ser vi nærmere på virksomhedsnetværket... (`gr_virk`). Lad os først lige se på netværkets komponent-struktur. Hvor mange komponenter består det af? 

```{r}
gr_virk <- gr_virk %>% 
  mutate(comp = group_components())

gr_virk %>% as_tibble() %>% count(comp) %>% head(10)

```

Vi kan nu bruge den nye variabel i vores `node-data` til at subsette netværket til den største komponent kun:

```{r}
largest_comp_virk <- gr_virk %>% filter(comp == 1)
```
...
Fordi vores netværks objekt er et tidygraph objekt, kan vi bruge en tidy-måde til at tilføje forskellige variable til `node-data` delen:

```{r, message=FALSE, warning=FALSE}
largest_comp_virk <- largest_comp_virk %>% 
...
Jeg har skrevet en plot-funktion der ligger i `correlationsplots.R` som kan downloads her:

[Download functions](functions/correlationplots.R){.btn .btn-primary}
...
Vi kan lige sørge for at closeness og betweenness målene er normaliseret.

```{r}
largest_comp_virk <- largest_comp_virk %>% 
  mutate(betweenness_norm = centrality_betweenness(normalized = T),
         closeness_norm = centrality_closeness(normalized = T))
```


```{r}
largest_comp_virk %>% as_tibble() %>% select(name, degree, closeness_norm, eigencentrality, betweenness_norm) %>%  cor_plots(., name_var = "name")
```

og vi kan se de parvise correlationer mellem målenen (vi 'smider' igen name-variablen ud `select(-name)`)

```{r}
largest_comp_virk %>% as_tibble() %>% select(degree, closeness_norm, eigencentrality, betweenness_norm) %>% cor(, method = "kendall")
...
```{r, warning=FALSE}
largest_comp_virk <- largest_comp_virk %>% 
  mutate(eccentricity = 1/node_eccentricity())
```
...
Lad os prøve at visualisere logikken med en funktion fra `coreness_viz.R`

som kan downloades her:

[Download functions](functions/coreness_viz.R){.btn .btn-primary}

og lægges i `functions`
...
largest_comp_virk %>% ggraph(layout='stress') + 
  ...
  geom_node_label(aes( filter=betweenness_rnk <= 10, label=name), alpha=0.65, size = 3, repel=T, force = 20)
```
...
largest_comp_virk %>% ggraph(layout='stress') + 
  ...
  geom_node_label(aes( filter=closeness_rnk <= 10, label=name), alpha=0.65, repel=T,size=3, force = 20)
...
stargr <- create_star(vcount(largest_comp_virk), mode = "undirected") 
