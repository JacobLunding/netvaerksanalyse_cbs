...
```{r}
pharma_nordic <- pharma_nordic %>% mutate(executive = grepl("BoD|ExeB|SenMan|ExeC", role_type) | 
...
net_virk <- net_virk %>% as_tbl_graph()
```
:::
...
::: g-col-6
```{r}
net_virk <- net_virk %>% mutate(comp = group_components())
net_virk %>% as_tibble(active = "nodes") %>% count(comp) %>% count(n)
```
...
::: g-col-6
```{r}
net_ind <- net_ind %>% mutate(comp = group_components())
net_ind %>% as_tibble(active = "nodes") %>% count(comp) %>% count(n)
```
...
::: g-col-6
```{r}
comp1_virk <- net_virk %>% filter(comp == 1)
comp2_virk <- net_virk %>% filter(comp == 2)
...
::: g-col-6
```{r}
comp1_ind <- net_ind %>% filter(comp == 1)
comp2_ind <- net_ind %>% filter(comp == 2)
```
:::
:::::
...
```{r}
net_virk <- net_virk %>% left_join(pharma_nordic %>% 
                            select(affiliation, 
                                   affiliation_country, 
                                   sector, 
                                   revenue,
                                   ...
                                   guo_country, 
                                   duo_name,
                                   duo_country) %>% distinct() %>%  rename(name= affiliation))
                    

```

```{r}
comp1_virk <- comp1_virk %>% left_join(pharma_nordic %>% 
                            select(affiliation, 
                                   affiliation_country, 
                                   sector, 
                                   revenue,
...
```{r}
comp2_virk <- comp2_virk %>% left_join(pharma_nordic %>% 
                            select(affiliation, 
                                   affiliation_country, 
                                   sector, 
                                   revenue,
...
```{r}
net_ind <- net_ind %>% left_join(pharma_nordic %>% 
                           select(name, 
                                  person_gender, 
                                  person_geo) %>% distinct(name, .keep_all = T))
...
```{r}
comp1_ind <- comp1_ind %>% left_join(pharma_nordic %>% 
                           select(name, 
                                  person_gender, 
                                  person_geo) %>% distinct(name, .keep_all = T))
```
...
```{r}
comp2_ind <- comp2_ind %>% left_join(pharma_nordic %>% 
                           select(name, 
                                  person_gender, 
                                  person_geo) %>% distinct(name, .keep_all = T))
```
...
`vcount()` tæller antallet af noder (vertices), `ecount()` tæller antallet af edges. `count_components` tæller antallet af komponenter
...
Vi har talt om trekantslukninger (**triadic closure**) (når A og B er venner *og* B og C er venner, er der sandsynlighed for at A og C også med tiden bliver det) og om hvordan det skaber closure i et netværk og bidrager til lokal densitet og cohesion - som skaber grundlag for tillid, sammenhold, norm-konformitet osv. Social kapital som gruppe fænomen.
...
```{r}
comp1_virk <- comp1_virk %>% mutate(
  degree      = centrality_degree(), 
  betweenness = centrality_betweenness(normalized = T),
  local_betweenness = centrality_betweenness(cutoff = 2, normalized = T),
...
comp1_virk <- comp1_virk %>% mutate(degree_rank          = dense_rank(desc(degree)),
...
cent_metrics <- comp1_virk %>% as_tibble(active = "nodes")
write_xlsx(cent_metrics, "output/pharma_nordic_example_nodes.xlsx")
...
```{r}
p <- comp1_virk %>% ggraph() +
  geom_edge_link0(width = 0.4, alpha = 0.3) +
  geom_node_point() +
  geom_node_point(aes(filter = brokerage_rank <= 5), color = "salmon2", size = 5) +
  geom_node_text(aes(filter = brokerage_rank <= 5, label = brokerage_rank)) +
  labs(caption = paste0(cent_metrics %>% arrange(brokerage_rank) %>% filter(brokerage_rank <= 5) %>% pull(brokerage_rank), ": ", 
                        cent_metrics %>% arrange(brokerage_rank) %>% filter(brokerage_rank <= 5) %>% pull(name), " (",
                        cent_metrics %>% arrange(brokerage_rank) %>% filter(brokerage_rank <= 5) %>% pull(constraint) %>% round(2), ")") %>% paste0(., collapse = "\n")) +
  theme_graph()
p
```
...
```{r}
assort <- comp1_virk %>% with_graph(tibble(employees  = graph_assortativity(attr = n_employees), # Kontinuert
...
Heller ikke den store 'effekt' her med undtagelse af country(!). Ikke så overraskende er der en høj assortativity her. Hvis I scroller op, kan i måske huske at den største komponent i høj grad var en komponent af svenske virksomheder.
...
```{r}
comp1_virk <- comp1_virk %>% mutate(case_when(is.na(guo_name)~name, .default = guo_name))
```
...
assort <- comp1_virk %>% with_graph(tibble(employees  = graph_assortativity(attr = n_employees), # Kontinuert
                                           ...
                                           assets     = graph_assortativity(attr = total_assets),# Kontinuert 
                                           country    = graph_assortativity(attr = affiliation_country),  # Nominiel
                                           owner      = graph_assortativity(attr = guo_name))) # Nominiel
