...
Til denne session skal I downloade:

scriptet:
[Scriptet - Session7_new.R](scripts%20til%20undervisning/Session7_new){.btn .btn-primary}

datasættet:
[Datasættet - finans_den17](data/finans_den17.csv){.btn .btn-primary}
som lægges i undermappen /data i din r-projekt-mappe

og funktionen:
[funktionen - get_edge_coord.R]("functions/get_edge_coord.R"){.btn .btn-primary}
...
```{r, warnings = F, messages = F}
bi_adj <- xtabs(den, formula = ~name + affiliation, sparse = T)
```
...
```{r, warnings = F, messages = F}
net_twomode <- bi_adj %>% 
  graph_from_biadjacency_matrix(, directed = FALSE, multiple = FALSE) %>% 
  simplify() %>% 
  as_tbl_graph()
net_twomode
```
...
```{r, warnings = F, messages = F}
net_twomode_l <- net_twomode %>% mutate(comp = group_components()) %>% filter(comp ==1)
net_ind_l     <- net_ind %>% mutate(comp = group_components()) %>% filter(comp ==1)
net_affil_l   <- net_affil %>% mutate(comp = group_components()) %>% filter(comp ==1)
```
...
```{r, warnings = F, messages = F}
net_affil_l <- net_affil_l %>% left_join(
  den %>% select(name = affiliation, type) %>% distinct())
```
...
vi kan også mappe sector henover color og lade både indhold og omkreds være styret af type

```{r, warnings = F, messages = F}
p + geom_node_point(size = 3, shape = 21, alpha = 0.75,
                    mapping = aes(color = type, fill = type))
```
...
```{r, warnings = F, messages = F}
colors <- c("Finance" = "deeppink4", "Banks" = "steelblue1", "Pension" = "forestgreen", "Leadership networks" ="steelblue4", "Insurance" = "salmon1")
```
...
```{r, warnings = F, messages = F}
p <- p + guides(fill = guide_legend(title = "Virksomhedstype"), 
           color = guide_legend(title = "Virksomhedstype"))
p
```
...
```{r, warnings = F, messages = F}
p + scale_color_manual(values = colors, breaks = c("Finance", "Insurance", "Banks", "Pension", "Leadership networks"), aesthetics = c("color", "fill"))
```
...
p <- p + scale_color_manual(values = colors, 
                            breaks = c("Finance", "Insurance", "Banks", "Pension", "Leadership networks"), 
                            labels = c("Finans", "Forsikring", "Banker", "Pension", "VL-grupper"), 
...
```{r, warnings = F, messages = F}
net_affil_l     <- net_affil_l %>% 
  mutate(bet = centrality_betweenness(normalized = T),
         bet_rnk = bet %>% desc %>% dense_rank)
...
```{r, warnings = F, messages = F}
net_affil_l     <- net_affil_l %>% 
  mutate(clo = centrality_closeness(normalized = T),
         clo_rnk = clo %>% desc %>% dense_rank)
```
...
p <- ggraph(net_affil_l) + 
  ...
  geom_node_point(shape = 21, alpha = 0.75,
                  mapping = aes(size = bet, fill = type)) + 
  scale_fill_brewer(name = "Virksomhedstype", palette = "Set1") +
...
UPS!!! Det går helt galt med edges, som nu ikke længere passer med noderne. Det er fordi koordinaterne nu er sorteret efter betweenness, mens 'referencen' til linjerne (from - to) stadig referer til de 'tidligere' række numre

Jeg har skrevet en funktion `get_edge_coord()` som sources med `get_edge_coord.R` 
...
metrics %>% 
ggplot() + 
  geom_point(aes(x = brokerage, y = betweenness, color=type, size=degree)) + 
...
metrics %>% 
ggplot() + 
  geom_density(aes(x = brokerage, fill=type)) + 
  labs(y='share') + 
  facet_wrap(~type) + 
