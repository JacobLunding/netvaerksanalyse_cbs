...
den_land <- den_land %>% group_by(person_name) %>% mutate(memberships = n_distinct(affiliation))
den_land <- den_land %>% group_by(affiliation) %>% mutate(members = n_distinct(person_name))
...
den_land <- den_land %>% filter(members < 30 & memberships > 1) 

bi_adj    <- xtabs(den_land, formula = ~person_name + affiliation, sparse = T)
...
members   <- den_land %>% group_by(affiliation) %>% summarise(n = n_distinct(person_name), landbrug = first(landbrug))
net       <- net %>% left_join(members %>% rename(name = affiliation))

net %>% filter(comp == 1) %>% ggraph() +
  geom_edge_link0(width = 0.3, alpha = 0.2) +
  geom_node_point(aes(color = landbrug)) +
  theme_graph()

net <- net %>% mutate(degree = centrality_degree(),
                      constraint = node_constraint(), 
                      betweennes = centrality_betweenness(),
                      ecc = node_eccentricity())


ego_net_plot(graph = net %>% filter(comp ==1), nodes = "Steen Bitsch 19735", mode = "constraint")
...
###################################################################################################/
comp1 <- add_vertex_attr(graph = comp1, data = cent_metrics, match_var = "affiliation")
...
# Omkod til gou country i stedet
V(comp1)$guo_name[is.na(V(comp1)$guo_name)] <- V(comp1)$name[is.na(V(comp1)$guo_name)]


assortativity_nominal(comp1, types = factor(V(comp1)$guo_country))
