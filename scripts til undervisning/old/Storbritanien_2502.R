...
########!!!!!!!! Der er et mismatch mellem jeres udvalg ovenfor og jeres color liste?
...
gr_UK <- add_vertex_attr(gr_UK, 
                         data = df_UK %>% select(affiliation, City_Category), 
...
gr_UK %>%
  ...
  geom_node_point(aes(color= City_Category)) +
...
plot1 + plot2     ## !!!!!!!!!!! Hvor kommer plot2 fra? 
# Komponenter & visualisering --------------------------------------------------
# Hvilke komponenter?
complist <- components(gr_UK)
# Dekomposering af grafen    ##!!!!!!!!!!!!!! Hvis I gerne vil have den største komponent kan I også bare bruge funktionen largest_component() i stedet for den lange kode med index osv.
...
df_UK_City <- df_UK %>% 
  filter(n_memberships > 2) #####!!!!!!!!!!! Her vælger I at folk skal have mere end 2 poster? Mener I mere end 1? For så skal det være enten > 1 eller >= 2
...
graph_uk_1 <- graph_from_data_frame(df_UK_City %>% select(name, affiliation), directed = F, vertices = graph_uk_1 %>% select())
