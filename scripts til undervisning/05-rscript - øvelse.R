...
# brug matrix multiplikation Matrix::t(incidence) %*% incidence eller
# eller brug funktionen incidence_to_adjacency( ) fra networkfunctions.R, hvor (..., mode = "col") betyder at den laver adjacency matricen for de enheder der er i incidencematricens kolonner (her affiliations) 
adj_virk  <- incidence_to_adjacency( .... )
...
comp <- add_vertrex_attr(graph = comp, 
                          data =  den_energi %>% 
...
p <- (gr_virk - which(degree(gr_virk) == 0)) %>% ggraph("fr") +
  ...
  geom_node_point(size = .5) + ggtitle(paste0("Energi (n=", vcount(gr_virk), ")")) +
  theme_graph(base_family = "serif")
p1 <- comp %>% ggraph("fr") +
  geom_edge_link0(width = 0.3, alpha = 0.3) +
  geom_node_point(aes(color = sector), size =2) + ggtitle(paste0("Største komponent (n=", vcount(comp), ")")) + guides(color = guide_legend("Country")) +
  theme_graph(base_family = "serif")
