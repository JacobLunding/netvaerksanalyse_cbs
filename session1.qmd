...
Først ser vi lige på hvad vores dataobjekt indeholder `glimpse()`. Det består af `81,429` og `30` kolonner - eller variable:

```{r}
den %>% glimpse()
```

Lad os tjekke hvad org_tags_full variablen indeholder med `count()` - og også med `distinct()`, da vores data indeholde flere rækker for hver organisation (`AFFILIATION`, som variablen med organisationsnavne hedder), da der er tale om et positionsdatasæt individ-rolle-organisation:

```{r}
den %>% count(org_tags, sort = TRUE)
...
```{r}
den <- den %>% mutate(landbrug_org  = str_detect(string = org_tags_full, pattern = regex("landbrug", ignore_case = TRUE)))
