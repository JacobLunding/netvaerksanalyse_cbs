...
Dernæst indlæser vi og gemmer datasættet `danish_elitenetworks2024.csv`, som ligger mappen `"/data"` i R projketet. Det kan også downloades her: [Download den24](data/danish_elitenetworks2024.csv){.btn .btn-primary}
...
Først ser vi lige på hvad vores dataobjekt indeholder `glimpse()`. Det består af `81,429` og `28` kolonner - eller variable:

```{r}
den %>% glimpse()
```

Lad os tjekke hvad `affiliation_tags` variablen indeholder med `count()` - og også med `distinct()`, da vores data indeholde flere rækker for hver organisation (`affiliation`, som variablen med organisationsnavne hedder), da der er tale om et positionsdatasæt individ-rolle-organisation:

```{r}
den %>% count(affiliation_tags, sort = TRUE)
den %>% distinct(affiliation, affiliation_tags) %>% count(affiliation_tags, sort = TRUE) 
```
Lad os sige vi gerne vil kigge på netværk i landbruget:
Til det har vi brug for at lave en variabel, der gør at vi kan identificere mødesteder, hvor ordet  "landbrug" indgår i et "tag"

Lad os lave to forskellige variable, som vi kan bruge til at finde landbrugsnetværket: én der fortæller os hvilke AFFILIATIONS, der har landbrug i tagget og én der fortæller os hvilke personer der er knyttet til (min.) en organisation/AFFILIATION, der er tagget som landbrug. Det gør vi ved at bruge `mutate()` - for at tilføje til vores data - og deri funktionen `str_detect()` og `regex()`. Med `str_detect` spørger vi om et string-pattern (her "landbrug") findes i en string variabel og med `regex()` fortæller vi funktionen at det er en regular expression, den skal lede efter og at den skal ignorere store/små bogstaver. Når vi har gjort det kan vi lave den anden variabel ved at gruppere `group_by` vores data på NAME (altså unikke individer) og igen i en `mutate()` spørge om landbrug_org er TRUE for bare én af dette individs positioner. Hvis det er sandt får dette individ værdien TRUE i alle sine rækker.

```{r}
den <- den %>% mutate(landbrug_org  = str_detect(string = affiliation_tags, pattern = regex("landbrug", ignore_case = TRUE)))
den <- den %>% group_by(person_name) %>% mutate(landbrug_pers = any(landbrug_org)) %>% ungroup()
```


Lad os bruge `filter()` til at lave et nyt dataobjekt, der kun indeholder de rækker med individer der har min. en position i noget der har med landbrug at gøre .
...
Netværksdata er ofte indsamlet i den form vi finder i `den24` datasættet. Altså som en såkaldt `edge-list`, der i to kolonner repræsenterer forbindelser mellem 'noget'. I dette personer i en kolonne og organisationer. Det kunne for så vidt også være individer i en kolonne og individer i den anden (fx. facebook venskaber). Når vi skal omforme vores data til et format der muliggør netværksanalyse og -visualisering bruger vi R-pakken `igraph`.
...
Individ x orgnisations matricen:
```{r, echo= TRUE}
x
```
den inverse matrice (`t(x)`):
```{r, echo= TRUE}
t(x)
```
